...
import { BagStatus, OfferStatus } from "@shared/types/bag-status";
import { getNextSaturdayDate } from "@shared/utils/get-next-saturday-date";

import { listBoxes } from "@cdd/_actions/boxes/GET/list-boxes";
...
export function FarmWithOrdersTable({ page, setTotalItems }: FarmsProps) {
  ...
  const [farms, setFarms] = useState<BoxDTO[]>([]);
  const [farmsFiltered, setFarmFiltered] = useState<BoxDTO[]>([]);
  ...
  const handleStatusFilterClick = (status: FilterStatus) => {
    ...
    setFarmFiltered(() =>
      farms.filter((item) => status.key.includes(item.status))
    );
      ...
      ? farmsFiltered.map((farm) => ({
          ...
          data: [
            ...
            { detail: farm.catalog.farm.name },
            {
              detail: getStatus({
                type: "oferta",
                status: farm.status as BagStatus["offer"],
              }),
            },
          ],
        }))
