...
export const secondStepRegisterSchema = z.object({
  cpf: z.string().min(1, { message: "CPF obrigatório" }).max(14).refine((value) => validateCPF(value), {
    message: "CPF não é válido"
  }),
  phone: z.string().min(1, { message: "Celular obrigatório" }).max(15).refine((value) => validateCellphone(value), {
    message: "Celular inválido.",
  }),
  password: z.string().min(1, "Senha obrigatória").min(8, "Senha deve ter pelo menos 8 caracteres."),
...
export const fifthStepRegisterSchema = z.object({
  ...
  tally: z.string().min(1, { message: "Número do talão é obrigatório" }),
})
